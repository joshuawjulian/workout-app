<script lang="ts">
	...
	let {
		...
		items: Item[];
		title: string;
		schema: { name: string; label: string; type: Field }[];
	...
	function getDisplayValue(item: Item, field: Field) {
		...
		if (field.type === 'multiselect') {
			...
			return value.map((v) => displayOptions.find((opt) => opt.value == v)?.label || v).join(', ');
		}
						...
						<tr>
							...
							<td class="text-right flex items-center justify-end">
								...
								<form
									...
									onsubmit={(e) => {
										if (
											!confirm(`Are you sure you want to delete "${item.name || 'this item'}"?`)
										) {
											e.preventDefault();
										}
									}}>
...
<dialog bind:this={dialog} id="crud_modal" class="modal" onclose={() => (selectedItem = null)}>
	<div class="modal-box">
		...
		<form method="POST" use:enhance={handleSubmit} class="py-4">
			{#if selectedItem}
			...
			{#each schema as field}
				<div class="form-control mt-4">
					...
					{:else if field.type === 'select'}
						<select id={field.name} name={field.name} class="select select-bordered" required>
							<option value="" disabled selected={selectedItem?.[field.name] == null}
								>Select one</option>
							{#each field.options ?? [] as option}
								<option value={option.value} selected={selectedItem?.[field.name] == option.value}
									>{option.label}</option>
							{/each}
						</select>
