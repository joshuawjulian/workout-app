...
CMD ["/bin/bash", "-c", "while [ ! -f package.json ]; do echo 'Waiting for project to be initialized...'; sleep 2; done; if [ -f package.json ]; then /sync-deps.sh && bun run dev --host 0.0.0.0; else echo 'No package.json found. Run: docker-compose exec app bun create svelte@latest . --template typescript'; while true; do sleep 30; done; fi"]